...
## (login) LOG: login failure for user
add chain=output action=accept protocol=tcp content="login failure for user" dst-limit=2/1m,3,dst-address/2m comment="bruteforce-blacklist" disabled=yes
add chain=output action=add-dst-to-address-list protocol=tcp content="login failure for user" address-list=_bruteforce_blacklist address-list-timeout=15m disabled=yes
